  ...
  return (
    <AppShell>
      <section className="mb-5 rounded-lg border border-primary/30 bg-card/70 p-5 shadow-node backdrop-blur">
        ...
        <h1 className="text-3xl font-black tracking-tight text-foreground">Unlock your life, one technology at a time.</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Habidoo is not a todo list. Start missions, respect cooldowns, and turn real effort into Life Tree progress.</p>
      ...
      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          <CardHeader><CardTitle className="flex items-center gap-2"><GitBranch size={17} className="text-primary" />Life Tree</CardTitle></CardHeader>
          <CardContent><p className="text-3xl font-black">{treeCompletion}%</p><p className="mt-2 text-xs text-muted-foreground">{state.completedTechnologyIds.length}/{technologies.length} technologies unlocked</p></CardContent>
        </Card>
        <Card>
      ...
      <section className="mt-5 grid gap-4 lg:grid-cols-[1.1fr_0.9fr]">
        <Card className="border-primary/25 bg-primary/5">
          <CardHeader><CardTitle className="flex items-center gap-2"><Zap size={17} className="text-strategy-gold" />Current Era</CardTitle></CardHeader>
          <CardContent>
            ...
            <div className="mt-4 rounded-md border border-border bg-background/45 p-3">
              <p className="text-xs font-black uppercase tracking-wide text-muted-foreground">Next unlockable technology</p>
              <p className="mt-1 font-bold text-foreground">{nextUnlock?.title ?? "Era mastery"}</p>
              <p className="mt-1 text-xs text-muted-foreground">{nextUnlock?.description ?? "Complete the remaining available branches."}</p>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="grid gap-4">
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><Timer size={17} className="text-primary" />Active Missions</CardTitle></CardHeader>
              ...
              {activeMissions.length ? activeMissions.map(([technologyId, runtime]) => {
                ...
                return (
                  <div key={technologyId} className="rounded-md border border-primary/35 bg-primary/10 p-3">
                    <p className="font-bold text-foreground">{tech?.title ?? "Life mission"}</p>
                    <p className="mt-1 text-xs text-muted-foreground">Started {runtime.startedAt ? new Date(runtime.startedAt).toLocaleTimeString() : "now"}</p>
                  </div>
                );
              }) : (
                <div className="rounded-md border border-border bg-muted/35 p-3">
                  <p className="font-bold text-foreground">No active mission</p>
                  <p className="mt-1 text-xs text-muted-foreground">Open the Life Tree and start one focused action.</p>
                </div>
              )}
            </CardContent>
          </Card>
