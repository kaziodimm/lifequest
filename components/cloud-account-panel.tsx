...
import { Cloud, LogOut, Mail, Save, UserRound } from "lucide-react";
...
export function CloudAccountPanel() {
  ...
  async function sendMagicLink() {
    ...
    setBusy(false);
    setMessage(error ? error.message : "Check your email for the Habidoo confirmation link.");
  }
  ...
  async function saveLocalProgress(userId = user?.id, silent = false) {
    ...
    setMessage(silent ? "Habid created and local progress saved to cloud." : "This device progress is saved to cloud.");
  }

  async function useCloudProgress() {
    if (!cloudState?.state) return;
    restoreCloudState(cloudState.state);
    setMessage("Cloud progress restored on this device.");
  }
  ...
  if (!configured) {
    return (
      <Card className="border-primary/30">
        <CardHeader><CardTitle className="flex items-center gap-2"><Cloud size={18} className="text-primary" />Cloud save</CardTitle></CardHeader>
        <CardContent className="grid gap-3 text-sm text-muted-foreground">
          <p>Guest/local mode is active. Add Supabase environment variables to enable Habidoo cloud accounts.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card className="border-primary/30">
      ...
      <CardContent className="grid gap-3">
        {!user ? (
          <>
            <p className="text-sm leading-6 text-muted-foreground">Save progress to an account. Habidoo sends a confirmation link before cloud save is enabled.</p>
            ...
            <Button disabled={busy || !email.trim()} onClick={sendMagicLink}><Mail size={16} />Send confirmation link</Button>
          </>
        ) : !confirmed ? (
          <div className="rounded-md border border-border bg-muted/35 p-3 text-sm leading-6 text-muted-foreground">Email is not confirmed yet. You can keep using local progress, but cloud save stays locked until confirmation.</div>
        ) : !profile ? (
          <>
            ...
            <Button disabled={busy || !habidValidation.valid} onClick={createProfile}><UserRound size={16} />Create Habid and save progress</Button>
          </>
        ) : (
          <>
            ...
            {cloudLoaded && cloudState ? (
              <div className="grid gap-2 rounded-md border border-primary/25 bg-primary/5 p-3 text-sm">
                <p className="font-bold text-foreground">Cloud progress found.</p>
                <p className="text-xs text-muted-foreground">Updated {new Date(cloudState.updated_at).toLocaleString()}.</p>
                <div className="grid gap-2 sm:grid-cols-2">
                  <Button variant="outline" disabled={busy} onClick={useCloudProgress}>Use cloud progress</Button>
                  <Button disabled={busy} onClick={() => saveLocalProgress()}><Save size={16} />Keep this device progress</Button>
                </div>
              </div>
            ) : null}
            {cloudLoaded && !cloudState ? <Button disabled={busy} onClick={() => saveLocalProgress()}><Save size={16} />Save local progress to cloud</Button> : null}
