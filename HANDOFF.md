...
## Closed-beta game shell pass — 2026-06-23

- Implemented the closed-beta shell requested by the latest prompt without changing the frozen Tree visual direction or adding new eras.
- Added Foundation 1-100 level helpers:
  - `lib/foundation-levels.ts`
  - `getEraLevelFromXp`, `getLevelProgress`, `getNextLevelXp`, `getFoundationLevelXpThreshold`.
  - Existing `levelFromXp` remains compatible through `lib/progression.ts`.
- Added structured game shell progression:
  - `lib/game-shell.ts` item catalog and achievement catalog.
  - `lib/game-shell-copy.ts` clean EN/RU/CS/UK copy for new game-shell UI.
  - New PlayerState fields: `unlockedInventoryItemIds`, equipped badge/frame/title, `betaTesterRewardGranted`, `treeGuideSeen`, `lastRewardEvent`.
  - Migration safely normalizes old saves and evaluates achievements from existing mission/focus/completed state.
- Added achievements:
  - first mission completed;
  - first focus object created;
  - first branch root completed;
  - three missions completed;
  - four branch roots completed;
  - Awakening Trial completed;
  - closed beta account.
- Added inventory/equipment:
  - beta tester badge/frame;
  - first mission badge;
  - focus seed badge;
  - root awakened badge;
  - three real steps badge;
  - Foundation Pathfinder frame;
  - Awakening title.
- Added beta tester one-time reward:
  - `CloudAccountPanel` grants it when a confirmed profile/Habid exists.
  - It does not duplicate and is included in Zustand/Supabase `user_game_state` snapshots.
- Updated Profile:
  - Foundation Level;
  - profile card with equipped frame/badge/title;
  - compact inventory with equip/unequip;
  - link to Rewards & Shop.
- Updated Awards:
  - Achievement Archive with unlocked and locked achievements;
  - hidden rewards only revealed after unlock;
  - Inventory Archive summary.
- Added `/rewards` protected page:
  - Shop coming soon;
  - current Research balances and Insight;
  - future locked reward categories without spending.
- Updated Tree shell:
  - first-run guide for the highlighted root mission;
  - restrained reward banner after mission completion/account reward;
  - mission panel remains focused.
- Updated brand:
  - generated premium logo concept saved at `public/brand/habidoo-logo-generated.png`;
  - crisp production favicon/logo SVG at `public/icon.svg`;
  - AppShell header and metadata use the main Habidoo logo/fav icon.
- Legal/product framing:
  - `/terms`, `/privacy`, `/rules` now use localized EN/RU/CS/UK `LegalPage`;
  - added beta limitation, account/data deletion wording, safety disclaimers, ownership/copyright wording, no fake patent/trademark claims, and support placeholder via invitation/support channel.
- Verification:
...
- Manual checks still recommended after deploy:
  ...
  - logout/login -> inventory/achievements restore from cloud;
  - mobile `/tree`, `/profile`, `/achievements`, `/rewards` no horizontal overflow.
